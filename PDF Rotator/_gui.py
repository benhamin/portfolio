...
class GUI:
  def __init__(self):
    super().__init__()
    self.root = tk.Tk()
    self.root.title('PDF Rotator {}'.format(Constants.APP_VERSION))
    self.root.withdraw()
    self.browse_file()
  ...
  def finished(self):
    mb.showinfo("PDF Rotator", "Processing Done! Opening Folder")

  def invalid_selected_file(self):
    mb.showinfo("PDF Rotator", "Invalid Selected File!")
